...
export default function NetworkrSolution() {
  ...
  return (
    <motion.section
      id="solution"
      className="py-12 md:py-20 lg:py-32 px-4 md:px-6 relative overflow-x-hidden overflow-y-hidden bg-linear-to-b from-surface/50 to-background"
      initial="hidden"
      whileInView="visible"
    ...
    >
      ...
      <div className="max-w-7xl mx-auto relative z-10 min-w-0">
        <div className="text-center mb-10 md:mb-16 lg:mb-20">
          <motion.div
            className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium mb-4 md:mb-6 border font-display"
            style={{
          ...
          <motion.h2
            className="text-2xl md:text-4xl lg:text-5xl font-light tracking-tight text-primary mb-4 md:mb-6 wrap-break-word font-display"
            variants={itemSection}
          >
            {solution.headline}
            <span className="font-display">
              {solution.headlineItalic}
            </span>
            <span className="font-display italic">
            {solution.universityName && `— ${solution.universityName}`}
            </span>
          </motion.h2>
          <motion.p
            className="text-base md:text-lg lg:text-xl text-secondary font-light max-w-2xl mx-auto wrap-break-word"
            variants={itemSection}
          >
            {solution.subtitle}
          </motion.p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
          {solution.cards.map((card) => (
            ...
            >
              <div
                className="p-4 md:p-6 lg:p-8 xl:p-10 rounded-2xl md:rounded-3xl border transition-all duration-500 h-full min-w-0
              ...
              >
                ...
                <h3 className="text-lg md:text-xl font-medium mb-3 md:mb-4 text-primary group-hover:text-primary-foreground wrap-break-word font-display">
                  {card.title}
                </h3>
                ...
                {card.bulletPoints && card.bulletPoints.length > 0 && (
                  <ul className="space-y-1.5 md:space-y-2 mb-4 md:mb-6 text-secondary group-hover:text-primary-foreground/90">
                    {card.bulletPoints.map((point) => (
                      ...
                      >
                        <Check className="w-3.5 h-3.5 md:w-4 md:h-4 shrink-0" strokeWidth={2} />
                        {point}
                      </li>
                    ))}
                  </ul>
