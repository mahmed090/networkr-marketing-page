...
export type StrategicValueIconName = 'trending-up' | 'users' | 'target' | 'award' | 'layers'
...
const defaultHero: HeroData = {
  ...
  card: {
    ...
    columns: [
      ...
      { title: 'Alumni', description: 'disengage from public platforms due to noise' },
      { title: 'Institutions', description: 'cannot fully activate their most valuable asset' },
    ],
  },
  bottomLine:
...
const defaultSolution: SolutionData = {
  ...
  cards: [
    ...
    },
    {
      ...
      learnMoreText: 'Learn more',
      bulletPoints: ['AI-powered matching', 'Career goal alignment', 'Industry & location based'],
      highlighted: true,
    },
    {
      icon: 'settings',
...
const defaultAboutNetworkr: AboutNetworkrData = {
  ...
  tagline: 'Networkr is purpose built alumni infrastructure designed specifically for universities.',
  stats: [
...
/** Per-tenant overrides (optional). Add entries for each tenant. */
const tenantDataOverrides: Partial<Record<string, Partial<SiteData>>> = {
  // example: 'acme': { brandName: 'Acme', hero: { ... } },
}
...
export function getSiteData(tenantId: string): SiteData {
  ...
  return {
    ...
    strategicValue: { ...defaultSiteData.strategicValue, ...overrides.strategicValue },
    implementation: { ...defaultSiteData.implementation, ...overrides.implementation },
    mattersNow: { ...defaultSiteData.mattersNow, ...overrides.mattersNow },
    partnerOpportunity: { ...defaultSiteData.partnerOpportunity, ...overrides.partnerOpportunity },
    aboutNetworkr: { ...defaultSiteData.aboutNetworkr, ...overrides.aboutNetworkr },
